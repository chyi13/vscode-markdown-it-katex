...
function inlineBareLatex(state: StateInline, silent: boolean): boolean {
...
function blockBareLatex(state: StateBlock, start: number, end: number, silent: boolean): boolean {
    var lastLine, next, lastPos, found = false, token,
        pos = state.bMarks[start] + state.tShift[start],
        max = state.eMarks[start]
...
export default function (md: import('markdown-it'), options?: MarkdownKatexOptions) {
    ...
    const enableMathBlockLatex = options?.enableMathBlockLatex;
    // #region Parsing
    ...
    if (enableMathBlockLatex) {
        md.inline.ruler.after('text', 'math_inline_bare_block_latex', inlineBareLatex);
    }
    ...
    md.block.ruler.after('blockquote', 'math_block', (state, start, end, silent) => {
        ...
        if (enableMathBlockLatex && blockBareLatex(state, start, end, silent)) {
            return true;
        }
